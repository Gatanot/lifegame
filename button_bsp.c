...
void Custom_ButtonInit(void) {
    ...
    button_init(&BootButton, read_button_GPIO, BOOT_Active, BOOT_ID);
    button_attach(&BootButton, BTN_SINGLE_CLICK, on_boot_single_click);
    button_attach(&BootButton, BTN_DOUBLE_CLICK, on_boot_double_click);
    button_attach(&BootButton, BTN_LONG_PRESS_START, on_boot_long_press_start);
